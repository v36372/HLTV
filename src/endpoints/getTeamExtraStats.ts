...
export const getTeamExtraStats = (config: HLTVConfig) => async ({
	...
	const f_ct$ = await fetchPage(`${config.hltvUrl}/stats/teams/ftu?${query}&side=COUNTER_TERRORIST`, config.loadPage)
	const p_ct$ = await fetchPage(`${config.hltvUrl}/stats/teams/pistols?${query}&side=COUNTER_TERRORIST`, config.loadPage)
	const f_t$ = await fetchPage(`${config.hltvUrl}/stats/teams/ftu?${query}&side=TERRORIST`, config.loadPage)
	const p_t$ = await fetchPage(`${config.hltvUrl}/stats/teams/pistols?${query}&side=TERRORIST`, config.loadPage)
	...
	for (var id in ftu_hash_ct) {
		if (pistol_hash_ct[id] !== null) {
			...
			teamsExtraStats.push({ team: team, ftu_ct: ftu_hash_ct[id], ftu_t: ftu_hash_t[id], pistol_ct: pistol_hash_ct[id], pistol_t: pistol_hash_t[id] })
		}
	}
