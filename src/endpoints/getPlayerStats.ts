...
export const getPlayerStats = (config: HLTVConfig) => async ({
  ...
  const i$ = await fetchPage(`${config.hltvUrl}/stats/players/individual/${id}/-?${query}`, config.loadPage)
  const c_1$ = await fetchPage(`${config.hltvUrl}/stats/players/clutches/${id}/1on1/-?${query}`, config.loadPage)
  const c_2$ = await fetchPage(`${config.hltvUrl}/stats/players/individual/${id}/1on2/-?${query}`, config.loadPage)
